...
public class Solution {
    public static void main(String[] args) {
        Set<? extends Animal> allAnimals = getAllAnimals(Solution.class.getProtectionDomain()
                .getCodeSource()
                .getLocation()
                .getPath() + Solution.class.getPackage()
                .getName()
                .replaceAll("[.]", "/") + "/data");
...
//        System.out.println(pathToAnimals);
        ...
        String[] pathes = dir.list((_dir, name) -> name.toLowerCase().endsWith(".class"));
        ...
        if (pathes != null) {
            for (String p : pathes) {
                try {
                    ...
                    result.add((Animal) clazz.getDeclaredConstructor().newInstance());
